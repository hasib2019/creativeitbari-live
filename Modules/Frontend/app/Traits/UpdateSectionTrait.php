...
trait UpdateSectionTrait {
    ...
    private function updateSectionOld($content, $request, array $fields, array $images = [], array $switches = [])
    {
        if (is_null($content)) {
            $content = new \stdClass();
        }
        ...
        foreach ($images as $image) {
            if ($request->hasFile($image)) {
                ...
                // Keep existing image
                // Do nothing — retains the old image
            } else {
                // No image uploaded and no old value
                $content->$image = null;
            }
        }
