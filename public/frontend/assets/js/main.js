...
(function ($) {
	...
	if($('#smooth-wrapper').length && $('#smooth-content').length && $('.prevent-smooth-scroll').length === 0) {
   ...
   // 76. perspective-slider animation //
    ...
    $('.password-show-toggle').each(function(){
        ...
        $this.on('click', function(){
            var inputType = $this.parent().find('input[type="password"], input[type="text"]');
            var openEye = $this.find('.open-eye-icon');
            var closeEye = $this.find('.close-eye-icon');
	...
	if ($("#header-currency-toggle").length > 0) {
		window.addEventListener('click', function(e){
			if (document.getElementById('header-currency-toggle').contains(e.target)){
				$(".header-currency-submenu").toggleClass("open");
			}
			else{
				$(".header-currency-submenu").removeClass("open");
			}
		});
	}
